...
def mass2plausibility(powerset, mass):
    ...
    """
    plausibility = np.zeros(len(powerset))
    for i, e1 in enumerate(powerset):
        for j, e2 in enumerate(powerset):
            if any(item in e1 for item in e2):
                plausibility[i] += mass[j]
    return plausibility
...
def jousselme_et_al(data):
    """
    Computes the Jousselme et al. entropy of the powerset, handling
    potential issues with zero values.
    ...
    for i in data.index:
        if np.sum(data.loc[i].element) == 1:
            bet = 0.0
            for j in data.index:
                if is_subset_new(data.loc[i].element, data.loc[j].element):
                    bet += data.loc[j].mass / data.loc[j].card

            epsilon = 1e-15  
            if bet > 0:  
                am += bet * np.log2(bet + epsilon)
    return -am
...
def jirousek_and_shenoy(data):
    """
    Computes the Jirousek and Shenoy entropy of the powerset, handling
    potential issues with zero plausibility values.
    ...
    if K == 0:  # Handle the case where all plausibilities are zero
        return dubois_prade(data) # Or an appropriate alternative

    pl_pm = np.array(pl_pm) / K

    epsilon = 1e-15  # Choose an appropriate small value
    log_pl_pm = np.where(pl_pm > 0, np.log2(pl_pm + epsilon), 0.0)  # Add epsilon *inside* log2
    return -np.dot(pl_pm, log_pl_pm) + dubois_prade(data)
...
def deng_and_wang(data):
    """
    Computes the Deng and Wang entropy of the powerset with enhanced numerical stability.
    ...
    epsilon : float, optional
        Small positive value to avoid taking square root of negative numbers.
        Default is 1e-10.

    Returns
    -------
    float
        The Deng and Wang entropy value.
    """
    du = 0.0
    epsilon=1e-15
    for i in data.index:
        if np.sum(data.loc[i].element) == 1:  # Iterate over singletons as before
            sqrt_arg = data.loc[i].belief + data.loc[i].plausibility + 1 - 2 * np.sqrt(max(0, data.loc[i].plausibility))
            du += 1 - np.sqrt(max(0, sqrt_arg) + epsilon)  # Ensure non-negative argument to sqrt and avoid sqrt(0)
...
def kavya_et_al(data):
    ...
    data = data[(data.card == 1) & (data.plausibility > 0)]
...
@namer("RAM (Relative Adjusted Mass)")
@period(4)
@classification("TU, EB")
def ram(data):
    """
    Computes the RAM (Relative Adjusted Mass) entropy 
    
    Returns
    -------
    float
        The RAM entropy value.
    """
    n = len(data.iloc[0]['element'])  # size of frame of discernment X
    rbp = np.zeros(n)  # one entry per singleton x_i ∈ X

    for i in range(n):  # for each singleton x_i
        for _, row in data.iterrows():
            if row['element'][i] == 1:  # if x_i ∈ A
                A_card = row['card']
                if A_card > 0:
                    factor = math.exp(1 - 2 ** ((n - A_card) / A_card))
                    rbp[i] += row['mass'] / A_card * factor

    # Remove zeros to avoid log(0)
    rbp = np.array([p for p in rbp if p > 0])
    return -np.sum(rbp * np.log2(rbp))
